...
class Answer_1{
    ...
    public static int solution(String[] crops) {
        int x_dim = crops.length;
        int y_dim = crops[0].length();
        int[][] crop_copy = new int[x_dim][y_dim];
        {
            int i = 0;
            while (i<x_dim) {
        ...
        return crop_copy[x_dim-1][y_dim-1];
    }
    ...
    public static int replant(String[] crops){
        ...
        int variable_1 = solution(crops);
        int variable_2 = solution(duplicate_crop);
        return Math.min(variable_1, variable_2);
    }
}